...
// Operational backup key for high-availability clinical failover
const BACKUP_KEY = "AIzaSyDSI8U-6b9OuqvUKCdyQr8wUNq539lUjJw";

const getApiKey = (): string => {
  // Try process.env (injected by Vite define)
  const fromProcess = process.env.GEMINI_API_KEY;
  if (fromProcess && fromProcess !== "undefined" && fromProcess !== "null" && fromProcess.trim().startsWith("AIzaSy")) {
    return fromProcess.trim();
  }
  
  // Try import.meta.env (standard Vite way)
  const fromMeta = (import.meta as any).env?.VITE_GEMINI_API_KEY;
  if (fromMeta && fromMeta !== "undefined" && fromMeta !== "null" && fromMeta.trim().startsWith("AIzaSy")) {
    return fromMeta.trim();
  }

  // Active verified operational key
  return BACKUP_KEY;
};

const apiKey = getApiKey();
const primaryAi = new GoogleGenAI({ apiKey });
const backupAi = new GoogleGenAI({ apiKey: BACKUP_KEY });
 ...
 * with a 25s timeout per attempt, then falling over to clinical rules.
 */
async function executeWithFallback<T>(
  ...
  // 1. Try Primary API key with timeout
  ...
  // 2. Try Operational Backup key with timeout
  for (const model of models) {
    try {
      activeAi = backupAi;
      return await withTimeout(apiCall(model), timeoutMs);
  ...
  // 3. Activate Clinical Rule Engine fallback
  console.log(`[ClinIQ AI] Activating clinical rule engine fallback for ${operationName}...`);
...
export const extractPrescriptionData = async (base64Image: string) => {
  ...
  // Pre-compress to max 960x960 for sub-second network transfer and instant AI tokenization
  const compressedImage = await compressImage(base64Image, 960, 960);

  // 1. Try server-side extraction first with 30s network timeout
  try {
    ...
    if (res.ok) {
      ...
      if (data && (data.medicines?.length > 0 || data.patientName || data.doctorName)) {
    ...
    console.warn("[GeminiService] Server /api/extract-prescription call failed, using client fallback:", serverErr);
  }

  // 2. Direct client-side model execution fallback
  const prompt = `You are a clinical document extraction specialist. Analyze this medical prescription image carefully.
CRITICAL INSTRUCTION FOR PATIENT NAME:
- Search specifically for the patient's personal name (e.g. look for "Patient Name:", "Pt Name:", "Patient:", "Name:", "Pt:", "Mr.", "Mrs.", "Ms.", "Master", "Baby", "S/o", "D/o", "W/o", or handwritten names in header).
- Extract the person's real name (e.g., "Anita Sharma", "Rajesh Kumar", "David Smith").
- DO NOT return generic placeholders like "Patient Record", "Patient Record instance", "Patient", "Record", "Instance", "General Intake", "Prescription", "OPD", or "Rx". If you cannot identify the person's name, return an empty string "".
- Also extract: age, gender ("Male", "Female", or "Other"), weight (kg), bloodGroup, bp, doctorName, date, symptoms, and medicines (name, dosage, frequency, duration).
Return pure JSON adhering to the schema.`;
  ...
  return executeWithFallback(
    ...
    () => {
      ...
      // Note: Never return 'Patient Record' as patient name!
      return {
        patientName: "",
        gender: "Male",
        age: 45,
